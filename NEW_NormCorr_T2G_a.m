...
for f_idx = 1:length(file_list)
    ...
    % --- NEW: Detect Original Bit-Depth ---
    % Get the MATLAB class (e.g., 'uint8' or 'uint16') of the very first raw frame
   ...
   %% Generate Time Vector and ROI Data from Bio-Formats Metadata
    ...
    % FIX: Account for ZEN "Original" vs "Uncompressed" metadata formatting
    markerTime_str = char(meta_hash.get('Global Information|TimelineTrack|TimelineElement|Time #1'));
    if isempty(markerTime_str)
        % Fallback for the collapsed "Original" save format
        markerTime_str = char(meta_hash.get('Global Information|TimelineTrack|TimelineElement|Time'));
    end
